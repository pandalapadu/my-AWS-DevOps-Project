...
```
### Configure the backend server configuration under NGINX server 
```bash
vim /etc/nginx/nginx.cong  ###this is main and Importenet Configurations for safety and security we willnot touch/edit here we will create /etc/nginx/default.d/expense.conf file need to be created 
```
---
